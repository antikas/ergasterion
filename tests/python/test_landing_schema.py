"""Assert-script tests for the Landing Product Contract wire schema (repo convention: no
...
Proves:
  ...
  - every positive vector in tests/fixtures/landing_schema_vectors.json round-trips
...
Usage:
    python tests/python/test_landing_schema.py
"""
...
# Allow direct execution as `python tests/python/test_landing_schema.py`.
...
SCHEMA_BUNDLE_PATH = REPO_ROOT / "ergasterion" / "schemas" / "landing-product-v1.schema.json"
EQUIVALENCE_PATH = REPO_ROOT / "ergasterion" / "schemas" / "landing-portable-idl-equivalence.json"
VECTORS_PATH = REPO_ROOT / "tests" / "fixtures" / "landing_schema_vectors.json"
...
def test_idl_hashes_to_the_pinned_digest():
    ...
    assert got == landing_contract.EXPECTED_IDL_SHA256, (
        f"docs/specifications/landing-portable-idl-v1.json hashes to {got}, "
        f"expected {landing_contract.EXPECTED_IDL_SHA256} -- the frozen structural "
...
def test_schema_bundle_regenerates_byte_identical():
    ...
    assert bundle["idl_sha256"] == landing_contract.EXPECTED_IDL_SHA256
...
def test_all_three_module_registries_are_disjoint_and_exhaustive():
    ...
    idl = records.load_idl(IDL_PATH)
    own = [set(landing_contract.RECORD_MODELS), set(runtime_binding.RECORD_MODELS), set(records.RECORD_MODELS)]
    union = set().union(*own)
...
def _model_for_vector(record_name: str):
    ...
    if record_name == "LogicalType":
        return landing_contract.SourceField
...
def test_positive_vectors_round_trip():
    vectors = _load_vectors()
    assert vectors["idl_sha256"] == landing_contract.EXPECTED_IDL_SHA256
...
def test_landing_product_contract_covers_all_three_delivery_modes():
    vectors = _load_vectors()
    modes = set()
    for vector in vectors["positive"]:
...
def test_empty_binary_golden_vector():
    ...
    scalar = landing_contract.TypedBinary.model_validate(golden["typed_value"])
    assert scalar.value == ""
...
def test_codecs_and_quality_modes_covered():
    vectors = _load_vectors()
    codec_kinds = {v["payload"]["landing"]["codec"]["kind"] for v in vectors["positive"] if v["record"] == "LandingProductContract"}
...
def test_modules_import_with_no_docs_directory_on_disk(tmp_check=True):
    ...
    contract_field = landing_contract.SourceField(name="f", logical_type="utf8_string", nullable=False)
...
def test_closed_models_reject_unknown_fields_across_all_three_modules():
    samples = [
        (landing_contract.LogicalIdentity, {"estate_namespace": "com.example.synthetic", "source": "a", "table": "b"}),
...
TESTS = [
    ...
    test_landing_product_contract_covers_all_three_delivery_modes,
