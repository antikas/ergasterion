"""Assert-script tests for Landing validation, lineage, metadata and publication.
...
Usage:
    python tests/python/test_landing_patterns.py
"""
...
VECTORS_PATH = REPO_ROOT / "tests" / "fixtures" / "landing_pattern_vectors.json"
...
def _append_batch_contract(base: LandingProductContract | None = None) -> LandingProductContract:
...
def _readiness(contract: LandingProductContract, manifest: str = DIGEST_A) -> InterfaceReadiness:
...
def test_derived_lineage_is_deterministic_and_binds_digests() -> None:
    contract = _sample_contract()
    plan = landing_execution_plan(contract)
...
def test_intent_precedes_confirmation_and_lineage_follows() -> None:
    ...
    payload = build_delivery_publication_payload(
        ...
        landing_partition_ref="accepted", accepted_content_digest=DIGEST_A,
    ...
    intent = build_projection_intent(
        ...
        projection_target="landing", projection_revision="1", originating_state_revision="0",
    ...
    _expect_error(
        ...
        lambda: require_observer_order((
            LifecycleEventType.LANDING_LINEAGE, LifecycleEventType.LANDING_PUBLICATION,
            LifecycleEventType.LANDING_QUALITY, LifecycleEventType.LANDING_QUARANTINE,
...
def test_omission_and_reordering_of_mandatory_occurrences_fail() -> None:
    ...
    checkpoint = next(item for item in plan.occurrences if item.occurrence_id == "landing.checkpoint")
    ...
    ingest = next(item for item in plan.occurrences if item.occurrence_id == "landing.ingest")
    publish = next(item for item in plan.occurrences if item.occurrence_id == "landing.publish")
    mutated = []
    for item in plan.occurrences:
        if item.occurrence_id == "landing.ingest":
            mutated.append(item.model_copy(update={"phase_ordinal": publish.phase_ordinal}))
        elif item.occurrence_id == "landing.publish":
    ...
    stripped = [
        item.model_copy(update={"members": ()}) if item.occurrence_id == "landing.checkpoint" else item
        for item in plan.occurrences
    ]
    _expect_error(
        "contract_invalid",
