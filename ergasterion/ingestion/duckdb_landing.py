...
One DuckDB file is the durable Landing store for candidate frames, the immutable
...
_KNOWN_LANDING: dict[str, str] = {}

_SCHEMA_SQL = """
CREATE TABLE IF NOT EXISTS schema_meta (
    version INTEGER NOT NULL,
    landing_written INTEGER NOT NULL DEFAULT 0
);
...
def _connect(path: Path):
    try:
        ...
        raise PortError("missing_extra", "duckdb is required for the local Landing adapters") from exc
...
class DuckDBStore:
    """One DuckDB file plus the closed Landing/projection/lifecycle schema."""
    ...
    def __init__(self, path: str | Path) -> None:
        ...
        known = _KNOWN_LANDING.get(resolved)
        try:
            self.conn = _connect(self.path)
        except Exception as exc:
            raise PortError("landing_store_restore_required", f"duckdb file is unreadable: {exc}") from exc
        self._ensure_schema()
        self._lost = False
        if not self._landing_tables_ok():
            self._lost = True
        elif known and not existed:
            self._lost = True
        elif known and not self.landing_written:
            self._lost = True
        elif self.landing_written and self._landing_row_count() == 0:
            self._lost = True

    @property
    def landing_written(self) -> bool:
        row = self.conn.execute("SELECT landing_written FROM schema_meta").fetchone()
    ...
    def _ensure_schema(self) -> None:
        ...
        try:
            ...
            if row is None:
                self.conn.execute(
                    "INSERT INTO schema_meta(version, landing_written) VALUES (?, 0)",
                    [SCHEMA_VERSION],
                )
            else:
                current = int(row[0])
    ...
    def _landing_tables_ok(self) -> bool:
        names = _table_names(self.conn)
        return all(name in names for name in LANDING_RELATIONS)

    def _landing_row_count(self) -> int:
        if not self._landing_tables_ok():
            return 0
        total = 0
    ...
    def mark_landing_written(self) -> None:
        self.conn.execute("UPDATE schema_meta SET landing_written = 1")
        _KNOWN_LANDING[str(self.path.resolve())] = "1"

    def require_available(self) -> None:
        if self._lost or not self._landing_tables_ok():
            raise PortError(
                "landing_store_restore_required",
                "landing partitions or the DuckDB file must be restored from backup",
            )
        if self.landing_written and self._landing_row_count() == 0:
            raise PortError(
                "landing_store_restore_required",
                "landing partitions were emptied and must be restored from backup",
            )

    def require_landing_lookup(self) -> None:
...
class DuckDBLandingAdapter:
    ...
    def finish_prepare(self, preparation: LandingPreparation) -> LandingEvidence:
        ...
        if existing is not None:
            return loads(LandingEvidence, existing["evidence_json"])
        row = self.store.fetchone(
            "SELECT * FROM preparations WHERE preparation_id = ?", [preparation.preparation_id],
        )
        if row is None:
        ...
        contract = LandingProductContract.model_validate(json.loads(row["contract_json"]))
        ...
        try:
            ...
            self.store.mark_landing_written()
            self.store.commit()
        except Exception:
            self.store.rollback()
            raise
    ...
    def read_candidate(self, query: CandidateReadQuery) -> CandidateFramePage:
        self.store.require_landing_lookup()
        partition = self.store.fetchone(
            "SELECT * FROM candidate_partitions WHERE candidate_ref = ?",
            [query.evidence.candidate_ref],
        )
        if partition is None:
            if self.store.landing_written or self.store._lost:
                raise PortError(
                    "landing_store_restore_required",
                    "landing candidate partition is missing and must be restored",
                )
    ...
    def begin_materialization(
        ...
        if existing is not None:
            stored = loads(LandingEvidence, existing["evidence_json"])
    ...
    def append_dispositions(self, session: MaterializationSession, page: DispositionPage) -> MaterializationSession:
        ...
        if int(page.first_frame_sequence) != expected:
            raise PortError("sequence_conflict", session.session_id)
        evidence = loads(LandingEvidence, row["evidence_json"])
        self.store.begin()
        try:
    ...
    def finish_materialization(self, completion: MaterializationCompletion) -> MaterializedLandingEvidence:
        ...
        if existing is not None:
            return loads(MaterializedLandingEvidence, existing["evidence_json"])
        evidence = loads(LandingEvidence, row["evidence_json"])
    ...
    def bind_release_visibility(self, binding: ReleaseVisibilityBinding) -> MaterializedLandingEvidence:
        ...
        if existing is not None:
            bound = loads(MaterializedLandingEvidence, existing["evidence_json"])
    ...
    def materialize_release(self, request: ReleaseMaterializationRequest) -> MaterializedLandingEvidence:
        ...
        prepared = loads(LandingEvidence, partition["evidence_json"])
        frame_rows = []
    ...
    def _frames(self, raw: bytes, contract: LandingProductContract, content_encoding: str) -> tuple[CandidateFrame, ...]:
