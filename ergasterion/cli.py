"""``ergasterion`` console entry point -- multiplexes factory emitters and Landing commands.

    ergasterion emit-products [args...] ->  ergasterion.emit_products:main
    ergasterion contracts [args...]     ->  ergasterion.emit_contracts:main
    ergasterion odps [args...]          ->  ergasterion.emit_odps:main
    ergasterion product-graph [args...] ->  ergasterion.emit_graph:main
    ...
    ergasterion validate [args...]      ->  ergasterion.framework.declaration:main
    ergasterion validate-canonical [args...] -> ergasterion.validate_canonical:main

Landing local-ingestion commands (lazy-imported so ``--help`` works without DuckDB):

    plan, contract, deployment, ingest, reconcile, local-backup, status, inspect, quarantine

Each emitter subcommand delegates to that module's existing ``main()``. Landing
...
SUBCOMMANDS: dict[str, str] = {
    "emit-products": "ergasterion.emit_products",
    "vault-rebaseline": "ergasterion.shapes.data_vault.rebaseline",
    "contracts": "ergasterion.emit_contracts",
    "odps": "ergasterion.emit_odps",
    "product-graph": "ergasterion.emit_graph",
    ...
    "validate": "ergasterion.framework.declaration",
    "validate-canonical": "ergasterion.validate_canonical",
...
def _usage() -> str:
    ...
    return (
        ...
        "the Landing contract or downstream interfaces.\n\n"
        ...
        "Landing commands require --project-dir PATH --source NAME --table KEY "
