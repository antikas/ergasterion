...
model_tests or vault_entities. See docs/specifications/landing-product-v1.md.
...
# --- explicit source mode: physical-schema -> Landing landing block ---------------------
#
...
# human (docs/specifications/landing-product-v1.md; ergasterion.source_delivery's draft
...
#
# Landing's LogicalType vocabulary (ergasterion.framework.landing_contract) is narrower than
...
# what the data means.
_LANDING_SIMPLE_LOGICAL: dict[str, str] = {
    ...
    # A nested/structured value (object family) has no Landing logical type of its own --
...
_LANDING_DECIMAL_BASE_TYPES = frozenset({
    "numeric", "decimal", "number", "float", "float4", "float8", "double", "real", "money",
})
_LANDING_LOCAL_DATETIME_BASE_TYPES = frozenset({"timestamp", "datetime", "time"})
_LANDING_UTC_INSTANT_BASE_TYPES = frozenset({"timestamptz"})
...
_ODCS_LOGICAL_TYPE_TO_LANDING: dict[str, str] = {
...
def _sql_type_to_landing_field(sql_type: str) -> Any:
    """One physical SQL/ODCS type string -> a Landing ``SourceField.logical_type`` value:
    ...
    if base in _LANDING_DECIMAL_BASE_TYPES:
    ...
    if base in _LANDING_LOCAL_DATETIME_BASE_TYPES:
        return {"kind": "local_datetime", "timezone": _DEFAULT_LOCAL_DATETIME_TIMEZONE}
    if base in _LANDING_UTC_INSTANT_BASE_TYPES:
        return "utc_instant"
    return _LANDING_SIMPLE_LOGICAL.get(base, "utf8_string")


def _odcs_logical_type_to_landing_field(logical_type: str | None) -> Any:
    return _ODCS_LOGICAL_TYPE_TO_LANDING.get((logical_type or "").lower(), "utf8_string")


_LANDING_CODEC_DEFAULTS: dict[str, dict[str, Any]] = {
    "csv": {
...
LANDING_CODEC_CHOICES = tuple(sorted(_LANDING_CODEC_DEFAULTS))
...
def build_source_landing(
    *,
    ...
    ``{name, logical_type, nullable}`` dicts already mapped onto the Landing vocabulary
    (see ``_sql_type_to_landing_field`` / ``_odcs_logical_type_to_landing_field``).
    ``source_name``/``identifier`` are folded through ``_slugify`` -- Landing's
    ``Identifier`` grammar is lowercase-only, and a draft that is already conformant
    needs no renaming to go to production later."""
    if codec_kind not in _LANDING_CODEC_DEFAULTS:
        raise ValueError(
            f"unsupported --codec {codec_kind!r}; expected one of {LANDING_CODEC_CHOICES}"
        )
    return {
        "kind": "source",
        ...
        "codec": dict(_LANDING_CODEC_DEFAULTS[codec_kind]),
...
def build_tables(
    ...
    *,
    ...
    Landing ``landing``/``delivery`` draft block; no ``raw_model``, ``seed_tests``,
    ...
    for schema_object in contract["schema"]:
        ...
        if landing_kind == "source":
            ...
            for prop in schema_object["properties"]:
                ...
                if physical_type:
                    logical_field = _sql_type_to_landing_field(str(physical_type))
                else:
                    logical_field = _odcs_logical_type_to_landing_field(prop.get("logicalType"))
...
def _source_landing_header(rel_path: str, contract: dict[str, Any]) -> str:
    return "\n".join([
        ...
        "# (properties, physicalType/logicalType, required) into a Landing landing block, with",
    ...
    *,
    ...
    if landing_kind == "source":
        ...
        header_lines = [
            ...
            "# and ergasterion/emit_contracts.py's outputs, this file is meant to be",
            "# hand-edited. It mechanically transcribes the contract's schema section",
            "# (properties, logical/physical types, keys) into projection stubs plus a",
            "# seed_tests/model_tests skeleton. What no ODCS contract carries is left as an",
            "# explicit TODO below and never guessed. Declare the products that read this",
            "# source once those TODOs are filled in.",
        ]
        header = "\n".join(header_lines)
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        help="seed (default): the unchanged vault-style declaration seed. source: emit a "
             "Landing landing/delivery draft carrying the physical schema alone -- no "
    ...
    if args.landing == "source":
        print(
            "Next: fill in the product / delivery / projection TODOs, including the "
            "table's product.domain, then flip delivery.kind to production -- see "
            "docs/specifications/landing-product-v1.md."
        )
    else:
        print("Next: declare the products that read this source under declarations/products/.")
    return 0
