"""Closed Pydantic projections of the frozen Landing portable IDL, vocabulary and
contract-declaration half.

The single structural authority is ``docs/specifications/landing-portable-idl-v1.json``
...
contract-declaration record family (``LandingProductContract`` and everything it is
...
(``tests/python/test_landing_schema.py`` calls them from a repository checkout); nothing
...
class ClosedModel(BaseModel):
    """Base for every Landing IDL record projection: closed (unknown fields rejected),
...
# --------------------------------------------------------------------------- scalars
#
# One Python type alias per IDL scalar (``docs/specifications/landing-portable-idl-v1.json``
...
ERROR_CODES: tuple[str, ...] = (
    ...
    "landing_store_restore_required", "capability_mismatch", "capacity_exceeded",
...
class LifecycleEventType(str, Enum):
    ...
    LANDING_SCHEMA = "landing.schema"
    LANDING_RECEIPT = "landing.receipt"
    LANDING_QUALITY = "landing.quality"
    LANDING_QUARANTINE = "landing.quarantine"
    LANDING_PUBLICATION = "landing.publication"
    LANDING_DELETION_EVIDENCE = "landing.deletion_evidence"
    LANDING_LINEAGE = "landing.lineage"
    LANDING_METADATA = "landing.metadata"
...
class LandingInterfaces(ClosedModel):
    raw: Token
    source_native: Token
    published: Token
    quarantine: Token
...
class LandingProductContract(ClosedModel):
    """The Landing Product Contract: a source's identity, product facts, source-native
    schema/codec, delivery policy and published projection, authored once (IDL record
    ``LandingProductContract``, schema token ``ergasterion.landing-product/v1``)."""

    schema_: Literal["ergasterion.landing-product/v1"] = Field(alias="schema")
...
# graph-shape digest does not cover. Access it as ``landing_contract.ExecutionPlan`` to
...
# chain (``landing_contract`` -> ``runtime_binding`` -> ``ingestion.records``) acyclic.
...
RECORD_MODELS: dict[str, type[BaseModel]] = {
    ...
    "LandingInterfaces": LandingInterfaces,
    "LandingProductContract": LandingProductContract,
