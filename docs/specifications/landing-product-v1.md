# Landing Product Contract specification v1
...
nullability rule applies to each field. Ergasterion's Landing IDL is one JSON file. It is
...
**Execution plan.** The resolved graph a Landing declaration compiles to: the pattern
...
implements each of the nine Landing ports, which target relations the projection writes to, and
the operating envelope (schedule, retry, resource ceilings, retention, protection profile).

## What the contract is

The **Landing Product Contract** is the one authored record that answers all three questions. A
...
`docs/specifications/landing-portable-idl-v1.json`, and one generated projection of it, the
JSON Schema bundle `ergasterion/schemas/landing-product-v1.schema.json`. Every field name,
...
Every Landing Product Contract carries, in one authored record:
...
- **Interfaces** (`interfaces`): the five named consumer-facing surfaces every Landing product
...
the landing schema and the projection mapping, plus the execution graph a Landing declaration
always resolves to. A contract author writes source facts and policy; the platform computes
lineage from that authoring.

## Field reference: `LandingProductContract` and its direct children

The frozen IDL's `records.LandingProductContract` entry and the matching JSON Schema bundle
...
| --- | --- | --- | --- | --- |
| `schema` | constant string `ergasterion.landing-product/v1` | yes | no | Fixes which wire shape this JSON document is. |
...
| `interfaces` | `LandingInterfaces` | yes | no | The five named consumer-facing surface identifiers. |
...
`tests/fixtures/landing_schema_vectors.json` validates against the live `LandingProductContract`
...
`tests/fixtures/landing_schema_vectors.json` under the note "production declaration 3/3".
...
value). `tests/fixtures/landing_schema_vectors.json` carries one positive vector per typed
...
it. The binding selects an adapter for each of nine Landing ports: source connector, raw
...
`tests/fixtures/landing_schema_vectors.json` carries one complete `RuntimeBinding` vector (all
...
Every Landing runtime interacts with its environment through exactly nine named ports. Each
...
`docs/specifications/landing-portable-idl-v1.json` `ports.<PortName>.methods` for each one.
...
tag each. `tests/python/test_landing_schema.py` recomputes both from scratch and asserts
...
`tests/fixtures/landing_schema_vectors.json` carries exactly this: an empty root page, a second
...
`tests/fixtures/landing_schema_vectors.json`, validated against the same Pydantic models in
`ergasterion/framework/landing_contract.py`.

## Vectors, coverage and the generated equivalence report

`tests/fixtures/landing_schema_vectors.json` holds two kinds of positive vector. Curated
...
`ergasterion/schemas/landing-portable-idl-equivalence.json` is the generated equivalence
...
`tests/python/test_landing_schema.py` runs.

`docs/specifications/landing-portable-idl-v1.json` is the frozen, hash-pinned structural
authority. Every record, field, type, union, port and error code originates here.

`ergasterion/schemas/landing-product-v1.schema.json` is the generated JSON Schema bundle, one
entry per IDL record, derived directly from the closed Pydantic models in
`ergasterion/framework/landing_contract.py`, `ergasterion/framework/runtime_binding.py` and
`ergasterion/ingestion/records.py`. Regenerating it from those models reproduces the committed
file byte for byte.

The equivalence report at `ergasterion/schemas/landing-portable-idl-equivalence.json`
